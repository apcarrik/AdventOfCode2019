...
func createComputer(intcodes *[]int64, cin, cfinished chan int64, phaseSetting int64, idx int, relativeBase int64) chan int64 {
...
func createPainterController(numAmps int, cfirstinput, clastoutput, cfinished chan int64, ccontrollerFinished chan bool) {
	...
	output := make([]int64, 2)
	panelGridWidth := 100
	panelGrid := make([][]int, panelGridWidth)
	for i := 0; i < panelGridWidth; i++ {
		panelGrid[i] = make([]int, panelGridWidth)
		for j := 0; j < panelGridWidth; j++ {
			panelGrid[i][j] = 0
		}
	}
	...
	for {
		...
		case lastOut = <-clastoutput:
			// go func() { cfirstamp <- lastOut }()
			fmt.Printf("last computer out: %d\n", lastOut)
			output[outputCounter] = lastOut
			outputCounter++
			if outputCounter > 1 {
				outputCounter = 0
...
func runProgram(program *[]int64, intcodes *[]int64, relativeBase int64) bool {
	...
	for i := 0; i < numComputers; i++ {
		cout = createComputer(intcodes, cout, cfinished, (*program)[i], i, relativeBase)
	}
	go createPainterController(numComputers, cin, cout, cfinished, ccontrollerFinished)
...
func main() {
	...
	program := []int64{1}
	output := runProgram(&program, intcodes, 0)
